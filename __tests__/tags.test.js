const { html, css } = require("../tags");

test("HTML tag should return plain html", () => {
	expect(html`<div>hello <span>world</span></div>`).toBe(
...
test("HTML in variables should be escaped", () => {
	...
	expect(html`<div>hello ${tomAndJerry}</div>`).toBe(
		"<div>hello &lt;span&gt;&quot;tom&quot; &amp; &#039;jerry&#039;&lt;/span&gt;</div>"
	);
});
...
test("CSS should return plain css", () => {
	expect(css`html body { color: black }`).toBe("html body { color: black }");
});

test("CSS in variables should return plain css", () => {
	const style = "{ color: black }";
	
	expect(css`html body ${style}`).toBe("html body { color: black }");
});
